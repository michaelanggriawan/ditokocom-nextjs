...
const ArrowBottom = (
	<svg xmlns='http://www.w3.org/2000/svg' width='35' height='35' viewBox='0 0 24 24' fill='none' stroke='#000000' strokeWidth='2.5' strokeLinecap='round' strokeLinejoin='round'>
		<path d='M6 9l6 6 6-6' />
	</svg>
);
...
export function BottomSheetOngkir({
	...
	return (
		<div className='ditoko__p0'>
			<div className={`${isHandleFocus ? 'ditoko__bottomSheetActive' : 'ditoko__bottomSheetNonActive'} ditoko__bottomSheetOngkir ${isBottomSheetOngkir ? 'ditoko__block' : 'ditoko__hidden'}`}>
				<div className='ditoko__flex ditoko__between'>
					<div onClick={handleCloseBsOngkir} className='ditoko__BsOngkirClose'>
						<img src='/images/arrowBack.png' alt='close' />
					</div>
				...
				<div className='ditoko__searchBsOrder'>
					<input onFocus={() => setHandleFocus(true)} onChange={(e) => setValueSearch(e.target.value)} value={isValueSearch} type='text' placeholder={`Cari ${selectName}`} />
				...
				{isLoadArea ? (
					...
					<div className={`${isHandleFocus ? 'ditoko__scrollDataAreaActive' : 'ditoko__scrollDataArea'} `}>
						{isDataArea
							...
							.map((items, i) => {
								return (
									<div key={i} onClick={selectDataArea} id={items.id} value={items.name} name={name} className='ditoko__listData ditoko__flex ditoko__between ditoko__alignCenter'>
...
const IconSearch = (
	<svg xmlns='http://www.w3.org/2000/svg' width='22' height='22' viewBox='0 0 24 24' fill='none' stroke='#767676' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
